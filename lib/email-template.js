...
var EmailTemplate = function () {
  ...
  _createClass(EmailTemplate, [{
    ...
    key: '_render',
    value: function _render(template, locals, locale, callback) {
      var _this3 = this;
      ...
      return this._init(locale).then(function () {
        return _this3._renderFile(template, locals, locale);
      }).tap(function () {
        return debug('Finished rendering ' + template);
      }).nodeify(callback);
    }
  }, {
    key: 'renderText',
    ...
    value: function renderHtml(locals, locale, callback) {
      ...
      return this._init(locale).then(function () {
        return _bluebird2.default.all([_this4._renderFile('html', locals, locale), _this4._renderStyle(locals, locale)]);
        ...
        if (_this4.options.disableJuice) return html;
    ...
    value: function _renderStyle(locals, locale) {
      ...
      return new _bluebird2.default(function (resolve) {
        // cached
        if (_this5.ltpls[locale].style !== undefined) {
          return resolve(_this5.ltpls[locale].style);
        }
        ...
        resolve(_this5._renderFile('style', locals, locale).then(function (style) {
          _this5.ltpls[locale].style = style;
    ...
    key: '_renderFile',
    value: function _renderFile(file, locals, locale) {
      var _this6 = this;

      return new _bluebird2.default(function (resolve, reject) {
        var preRenderHook = _bluebird2.default.resolve(null);

        if (_this6.options.preRenderHook && (0, _isFunction2.default)(_this6.options.preRenderHook)) {
          preRenderHook = _this6.options.preRenderHook(file, _this6.ltpls[locale].files, locals, locale);
        }

        preRenderHook.then(function (preRenderResult) {
          if (preRenderResult) {
            _this6.ltpls[locale].files[file] = preRenderResult;
          }

          if (!_this6.ltpls[locale].files[file]) return resolve(null);

          (0, _util.renderFile)(_this6.ltpls[locale].files[file], locals).then(function (rendered) {
            var postRenderHook = _bluebird2.default.resolve(rendered);

            if (_this6.options.postRenderHook && (0, _isFunction2.default)(_this6.options.postRenderHook)) {
              postRenderHook = _this6.options.postRenderHook(rendered, file, _this6.ltpls[locale].files, locals, locale);
            }
